...
public class event_generator_zone {
    ...
    private void generate_events(File outputs) {
    ...
    private void generate_zones(File outputs) {
        TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
        Date date = this.get_date();
        String status = "SCHEDULE";
        int type;
        ...
        try (FileWriter file = new FileWriter(outputs)) {
            ...
            file.write("  \"start\":\"" + new Date(dateL - (5 * 3600 * 100)) + "\",");
            ...
            file.write("  \"end\":\"" + new Date(dateL + (13 * 3600 * 100)) + "\",");
            file.write("\"data\":{");
            ...
            file.write("  {\"id\":\"" + UUID.randomUUID() + "\",");
            file.write("  \"zone\":\"" + "zone" + "\",");
            file.write("  \"start\":\"" + new Date(dateL + (17 * 3600 * 100)) + "\",");
            file.write("  \"end\":\"" + new Date(dateL + (22 * 3600 * 100)) + "\",");
            file.write("\"data\":{");
    ...
    public static void main(String... args) throws IOException {
        ...
        for (int d = 0; d < configurations.size(); d++) {
            ...
            File file_events = new File(data_model + "/events.json");
            File file_zone = new File(data_model + "/zones.json");
            event_generator_zone events = new event_generator_zone(namespace, date);
            events.generate_zones(file_zone);
            events.generate_events(file_events);
